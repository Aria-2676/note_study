...
class _ScratchCardPageState extends State<ScratchCardPage> {
  ...
  Future<void> _loadCustomPrizePool() async {
    ...
    if (pool.isEmpty) {
      for (final item in provider.shopItems) {
  ...
  Future<void> _loadLotteryRecords() async {
    try {
      ...
      print('Failed to load lottery records: $e');
    }
  }
  ...
  Future<void> _startScratch(AppProvider provider) async {
    ...
    if (provider.currentPoints < _selectedCost) {
  ...
  Widget build(BuildContext context) {
    ...
    final availableItems = provider.shopItems
        .where(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              ElevatedButton(
                onPressed: () => _startScratch(provider),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFFFF6B6B),
